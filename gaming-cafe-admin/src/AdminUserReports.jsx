...
export default function AdminUserReports() {
  ...
  const loadReports = async () => {
    try {
      ...
      const response = await fetch(`${ADMIN_API_BASE}/api/admin/reports`, {
        method: 'GET',
        headers: {
  ...
  const handleStatusChange = async (reportId, newStatus) => {
    if (window.confirm(`Are you sure you want to change this report status to ${newStatus}?`)) {
      try {
        ...
        const response = await fetch(`${ADMIN_API_BASE}/api/admin/reports/${reportId}/status`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
  ...
  const handleAddResponse = async (reportId) => {
    ...
    try {
      ...
      const response = await fetch(`${ADMIN_API_BASE}/api/admin/reports/${reportId}/response`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
